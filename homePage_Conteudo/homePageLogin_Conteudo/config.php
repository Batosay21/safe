...
function createUserTables($conn) {
    ...
    // Tabela de produtos favoritos (NOVA)
    ...
    // Verificar e adicionar colunas se não existirem (para compatibilidade com instalações existentes)
    try {
        ...
        // Ignorar erros se a tabela favoritos não existir ainda
        if (strpos($e->getMessage(), "favoritos' doesn't exist") === false) {
            error_log("Erro ao verificar/atualizar tabela favoritos: " . $e->getMessage());
        }
    }
}
